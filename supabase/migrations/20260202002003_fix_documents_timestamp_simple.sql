-- Simple fix: Change d.updated_at to d.created_at in search function
-- Only modifying the final SELECT, not the entire function

DROP FUNCTION IF EXISTS search_all_entities_hybrid(text, vector, uuid, text[], float, int);
...
BEGIN
  ...
  -- Final SELECT with fixed timestamp columns
  SELECT
    f.entity_type,
    f.entity_id,
    CASE f.entity_type
    ...
    END as metadata,
    -- FIX: Use created_at for documents (doesn't have updated_at)
    CASE f.entity_type
...
COMMENT ON FUNCTION search_all_entities_hybrid IS 'Unified semantic search with RRF - fixed documents.created_at';
